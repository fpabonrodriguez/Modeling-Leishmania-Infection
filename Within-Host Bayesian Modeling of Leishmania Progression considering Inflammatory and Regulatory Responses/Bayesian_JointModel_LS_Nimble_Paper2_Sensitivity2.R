...
# Creating clusters
seed_sens2 <- 82302   # sensitivity analysis 2   dinvgamma(shape=2, scale=0.5)
ncore <- 3    
cl <- makeCluster(ncore, outfile = "log_run_sens2.log")
clusterSetRNGStream(cl, iseed = seed_sens2)
registerDoParallel(cl)
filename_results <- "results_sens2.rds"
...
BayesianModel <- nimbleCode({
  ...
  sigma2.betaP ~ dinvgamma(shape=2, scale=0.5)
  sigma2.betaA ~ dinvgamma(shape=2, scale=0.5)
  sigma2.betaD2 ~ dinvgamma(shape=2, scale=0.5)
  sigma2.betaD3 ~ dinvgamma(shape=2, scale=0.5)
  sigma2.betaI1 ~ dinvgamma(shape=2, scale=0.5)
  sigma2.betaI2 ~ dinvgamma(shape=2, scale=0.5)
  sigma2.betaI3 ~ dinvgamma(shape=2, scale=0.5)
  sigma2.betaR1 ~ dinvgamma(shape=2, scale=0.5)
  sigma2.betaR2 ~ dinvgamma(shape=2, scale=0.5)
  sigma2.betaR3 ~ dinvgamma(shape=2, scale=0.5)
  ...
  # Survival SubModel
  sigma2.gammaS ~ dinvgamma(shape=2, scale=0.5)
  for(i in 1:ncolsX){
...
time2 <- Sys.time() 
(runtime <- time2-time1)
saveRDS(results_fit, file = filename_results)
stopImplicitCluster()
stopCluster(cl)
...
if(want.results == TRUE){
  # Results
  results_file <- readRDS(file = "results_sens2.rds")
  ...
  MCMCtrace(results_mcmc, 
            ...
            pdf = TRUE,
            Rhat = TRUE,
            filename = "MCMCtrace_parameters_sens2.pdf")
  ...
  MCMCtrace(results_mcmc, 
            ...
            Rhat = TRUE,
            exact = TRUE,
            filename = "MCMCtrace_w_parameters_sens2.pdf")
  ...
  MCMCtrace(results_mcmc, 
            ...
            pdf = TRUE,
            Rhat = TRUE,
            filename = "MCMCtrace_latent_sens2.pdf")
  ...
  MCMCtrace(results_mcmc, 
            ...
            pdf = TRUE,
            Rhat = TRUE,
            filename = "MCMCtrace_logHaz_logSurv_sens2.pdf")
