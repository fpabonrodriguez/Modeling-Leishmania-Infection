...
for(i in 1:nsim){
  ...
  alphaD1 <- combined_list[id_sim,grepl("alphaD1",colnames(combined_list))]
  ...
  for(t in 3:time_pred){
    ...
    # Disease status (D)
    exp_term1 <- c(exp(M1%*%betaD1 + mX[1,]%*%alphaD1 + wDt[1] + thetaD*wDtm1[1]), 
                   exp(M1%*%betaD2 + mX[1,]%*%alphaD2 + wDt[1] + thetaD*wDtm1[1]),
                   exp(M1%*%betaD3 + mX[1,]%*%alphaD3 + wDt[1] + thetaD*wDtm1[1]))
    total_term1 <- 1 + sum(exp_term1)
    prob1 <- c(exp_term1/total_term1,1-sum(exp_term1/total_term1)) 
    pred_D_subj1[i,t] <- rcat(1,prob = prob1)
    
    exp_term2 <- c(exp(M2%*%betaD1 + mX[2,]%*%alphaD1 + wDt[2] + thetaD*wDtm1[2]), 
                   exp(M2%*%betaD2 + mX[2,]%*%alphaD2 + wDt[2] + thetaD*wDtm1[2]),
                   exp(M2%*%betaD3 + mX[2,]%*%alphaD3 + wDt[2] + thetaD*wDtm1[2]))
    total_term2 <- 1 + sum(exp_term2)
    prob2 <- c(exp_term2/total_term2,1-sum(exp_term2/total_term2)) 
    pred_D_subj2[i,t] <- rcat(1,prob = prob2)
    
    exp_term3 <- c(exp(M3%*%betaD1 + mX[3,]%*%alphaD1 + wDt[3] + thetaD*wDtm1[3]), 
                   exp(M3%*%betaD2 + mX[3,]%*%alphaD2 + wDt[3] + thetaD*wDtm1[3]),
                   exp(M3%*%betaD3 + mX[3,]%*%alphaD3 + wDt[3] + thetaD*wDtm1[3]))
    total_term3 <- 1 + sum(exp_term3)
    prob3 <- c(exp_term3/total_term3,1-sum(exp_term3/total_term3)) 
    pred_D_subj3[i,t] <- rcat(1,prob = prob3)
    
    exp_term4 <- c(exp(M4%*%betaD1 + mX[4,]%*%alphaD1 + wDt[4] + thetaD*wDtm1[4]), 
                   exp(M4%*%betaD2 + mX[4,]%*%alphaD2 + wDt[4] + thetaD*wDtm1[4]),
                   exp(M4%*%betaD3 + mX[4,]%*%alphaD3 + wDt[4] + thetaD*wDtm1[4]))
...
pred.plot <- function(subj = 1, 
  ...
  grid(NULL, NULL, col = "white")
  legend("topleft", legend=c("Median", "Observed", "95% CI Bounds"), 
  ...
  lines(2:pred_time, df[4,2:pred_time], col = "darkorange3", lwd = 4, lty = 4)
  legend("topleft", legend=c("D=1","D=2","D=3","D=4"), 
         lty = c(1,2,3,4), lwd = 3, col = c("darkorange", "darkorange1", 
