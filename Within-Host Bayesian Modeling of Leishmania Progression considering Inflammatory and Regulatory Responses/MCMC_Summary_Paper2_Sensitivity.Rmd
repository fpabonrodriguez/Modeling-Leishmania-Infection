...
for(i in 1:length(clnamesSD)){
    
    df <- data.frame(combined_list[,clnamesSD[i]],
                 combined_list1[,clnamesSD[i]],
                 combined_list2[,clnamesSD[i]])
    colnames(df) <- c("Gamma","Uniform", "InvGamma")
    dfm <- melt(df)
    ...
    print(ggplot(dfm, aes(x=value, fill=variable)) +
       ...
       scale_fill_manual(values=c("#F8766D", "#00BA38", "#619CFF"), 
                         name="Prior Selection",
                         labels=colnames(df)) + 
       ggtitle(paste0("Posterior Density Plot of ",clnamesSD[i])))
    Sys.sleep(2)
