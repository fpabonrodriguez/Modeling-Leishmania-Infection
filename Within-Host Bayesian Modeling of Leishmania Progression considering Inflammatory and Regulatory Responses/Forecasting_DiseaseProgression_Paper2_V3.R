...
# Combine MCMC chains
results_file <- readRDS("results_fit_V3.rds")
...
for(i in 1:nsim){
  ...
  for(t in 3:time_pred){
    ...
    # Disease status (D)
    exp_term1 <- c(exp(M1%*%betaD1 + mX[1,]%*%alphaD1), 
                   exp(M1%*%betaD2 + mX[1,]%*%alphaD2),
    ...
    exp_term2 <- c(exp(M2%*%betaD1 + mX[2,]%*%alphaD1), 
                   exp(M2%*%betaD2 + mX[2,]%*%alphaD2),
    ...
    exp_term3 <- c(exp(M3%*%betaD1 + mX[3,]%*%alphaD1), 
                   exp(M3%*%betaD2 + mX[3,]%*%alphaD2),
    ...
    exp_term4 <- c(exp(M4%*%betaD1 + mX[4,]%*%alphaD1), 
                   exp(M4%*%betaD2 + mX[4,]%*%alphaD2),
