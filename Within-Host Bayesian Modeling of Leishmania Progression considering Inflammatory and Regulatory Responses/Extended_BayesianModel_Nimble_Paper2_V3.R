...
# Creating clusters
ncore <- 3    
cl <- makeCluster(ncore, outfile = "log_file_Updated_V3.txt")
  ...
  for(i in 1:N){
    for(t in (first_timepoint[i]+1):last_timepoint[i]){
      ...
      muD1[i,t] <- inprod(M[i,1:ncolsM,t-1],betaD1[1:ncolsM]) + 
        inprod(X[i,1:ncolsX],alphaD1[1:ncolsX])
...
time1 <- Sys.time() 
results_fit <- foreach(x = as.numeric(paste0("202208",1:ncore)), 
...
time2 <- Sys.time() 
(runtime <- time2-time1)
saveRDS(results_fit, file = "results_fit_V3.rds", version = 2)
stopImplicitCluster()
stopCluster(cl)
