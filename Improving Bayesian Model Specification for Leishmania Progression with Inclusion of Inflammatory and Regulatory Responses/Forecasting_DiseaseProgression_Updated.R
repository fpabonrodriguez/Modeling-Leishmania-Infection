...
dt_P <- ifelse(dt_P == 0, 1e-06, dt_P)  
dt_P <- log(dt_P / (1 - dt_P))
dt_Pc <- log(dt_Pc / (1 - dt_Pc))
dt_A <- log(dt_A / (1 - dt_A))

dt_I1 <- ifelse(dt_I1 == 0, 1e-04, dt_I1)  
dt_I2 <- ifelse(dt_I2 == 0, 1e-04, dt_I2) 
dt_I3 <- ifelse(dt_I3 == 0, 1e-04, dt_I3) 
dt_R1 <- ifelse(dt_R1 == 0, 1e-04, dt_R1) 
dt_R2 <- ifelse(dt_R2 == 0, 1e-04, dt_R2) 
dt_R3 <- ifelse(dt_R3 == 0, 1e-04, dt_R3) 
...
pred_P_subj1[,1:2] <- cbind(rep(mP[1,1],nsim),rep(mP[1,2],nsim))
pred_P_subj2[,1:2] <- cbind(rep(mP[2,1],nsim),rep(mP[2,2],nsim)) 
pred_P_subj3[,1:2] <- cbind(rep(mP[3,1],nsim),rep(mP[3,2],nsim)) 
pred_P_subj4[,1:2] <- cbind(rep(mP[4,1],nsim),rep(mP[4,2],nsim)) 
...
for(i in 1:nsim){
  for(t in 3:time_pred){
    ...
    betaD1 <- combined_list[id_sim,grepl("betaD1",colnames(combined_list))]
    ...
    alphaD <- combined_list[id_sim,grepl("alphaD",colnames(combined_list))]
    ...
    thetaP <- (combined_list[id_sim,grepl("thetaP",colnames(combined_list))])[1]
    thetaA <- combined_list[id_sim,grepl("thetaA",colnames(combined_list))]
    thetaD <- combined_list[id_sim,grepl("thetaD",colnames(combined_list))]
    thetaIR <- matrix(combined_list[id_sim,grepl("thetaIR",colnames(combined_list))],6,6)
    wPt <- combined_list[id_sim,paste0("wP","[",c(36,43,46,49),", ",t,"]")]
    wAt <- combined_list[id_sim,paste0("wA","[",c(36,43,46,49),", ",t,"]")]
    wDt <- combined_list[id_sim,paste0("wD","[",c(36,43,46,49),", ",t,"]")]
    wPtm1 <- combined_list[id_sim,paste0("wP","[",c(36,43,46,49),", ",t-1,"]")]
    wAtm1 <- combined_list[id_sim,paste0("wA","[",c(36,43,46,49),", ",t-1,"]")]
    wDtm1 <- combined_list[id_sim,paste0("wD","[",c(36,43,46,49),", ",t-1,"]")]
    wIRt_1 <- combined_list[id_sim,paste0("wIR","[",36,", ",1:6,", ", t,"]")]
    wIRt_2 <- combined_list[id_sim,paste0("wIR","[",43,", ",1:6,", ", t,"]")]
    wIRt_3 <- combined_list[id_sim,paste0("wIR","[",46,", ",1:6,", ", t,"]")]
    wIRt_4 <- combined_list[id_sim,paste0("wIR","[",49,", ",1:6,", ", t,"]")]
    ...
    # Pathogen load (P)
    pred_P_subj1[i,t] <- rnorm(1, mean = M1%*%betaP + mX[1,]%*%alphaP + wPt[1] + thetaP*wPtm1[1],
                               sd = sigmaP)
    pred_P_subj2[i,t] <- rnorm(1, mean = M2%*%betaP + mX[2,]%*%alphaP + wPt[2] + thetaP*wPtm1[2],
                               sd = sigmaP)
    pred_P_subj3[i,t] <- rnorm(1, mean = M3%*%betaP + mX[3,]%*%alphaP + wPt[3] + thetaP*wPtm1[3],
                               sd = sigmaP)
    pred_P_subj4[i,t] <- rnorm(1, mean = M4%*%betaP + mX[4,]%*%alphaP + wPt[4] + thetaP*wPtm1[4],
                               sd = sigmaP)
    
    # Antibody levels (A)
    pred_A_subj1[i,t] <- rnorm(1,mean = M1%*%betaA + mX[1,]%*%alphaA + wAt[1] + thetaA*wAtm1[1], 
                               sd = sigmaA)
    pred_A_subj2[i,t] <- rnorm(1,mean = M2%*%betaA + mX[2,]%*%alphaA + wAt[2] + thetaA*wAtm1[2], 
                               sd = sigmaA)
    pred_A_subj3[i,t] <- rnorm(1,mean = M3%*%betaA + mX[3,]%*%alphaA + wAt[3] + thetaA*wAtm1[3], 
                               sd = sigmaA)
    pred_A_subj4[i,t] <- rnorm(1,mean = M4%*%betaA + mX[4,]%*%alphaA + wAt[4] + thetaA*wAtm1[4], 
                               sd = sigmaA)
    
    # Disease status (D)
    exp_term1 <- c(exp(M1%*%betaD1 + mX[1,]%*%alphaD + wDt[1] + thetaD*wDtm1[1]), 
                   exp(M1%*%betaD2 + mX[1,]%*%alphaD + wDt[1] + thetaD*wDtm1[1]),
                   exp(M1%*%betaD3 + mX[1,]%*%alphaD + wDt[1] + thetaD*wDtm1[1]))
    total_term1 <- 1 + sum(exp_term1)
    prob1 <- c(exp_term1/total_term1,1-sum(exp_term1/total_term1)) 
    pred_D_subj1[i,t] <- rcat(1,prob = prob1)
    
    exp_term2 <- c(exp(M2%*%betaD1 + mX[2,]%*%alphaD + wDt[2] + thetaD*wDtm1[2]), 
                   exp(M2%*%betaD2 + mX[2,]%*%alphaD + wDt[2] + thetaD*wDtm1[2]),
                   exp(M2%*%betaD3 + mX[2,]%*%alphaD + wDt[2] + thetaD*wDtm1[2]))
    total_term2 <- 1 + sum(exp_term2)
    prob2 <- c(exp_term2/total_term2,1-sum(exp_term2/total_term2)) 
    pred_D_subj2[i,t] <- rcat(1,prob = prob2)
    
    exp_term3 <- c(exp(M3%*%betaD1 + mX[3,]%*%alphaD + wDt[3] + thetaD*wDtm1[3]), 
                   exp(M3%*%betaD2 + mX[3,]%*%alphaD + wDt[3] + thetaD*wDtm1[3]),
                   exp(M3%*%betaD3 + mX[3,]%*%alphaD + wDt[3] + thetaD*wDtm1[3]))
    total_term3 <- 1 + sum(exp_term3)
    prob3 <- c(exp_term3/total_term3,1-sum(exp_term3/total_term3)) 
    pred_D_subj3[i,t] <- rcat(1,prob = prob3)
    
    exp_term4 <- c(exp(M4%*%betaD1 + mX[4,]%*%alphaD + wDt[4] + thetaD*wDtm1[4]), 
                   exp(M4%*%betaD2 + mX[4,]%*%alphaD + wDt[4] + thetaD*wDtm1[4]),
                   exp(M4%*%betaD3 + mX[4,]%*%alphaD + wDt[4] + thetaD*wDtm1[4]))
    total_term4 <- 1 + sum(exp_term4)
    prob4 <- c(exp_term4/total_term4,1-sum(exp_term4/total_term4)) 
    ...
    pred_IR_subj1[i,,t] <- rmvnorm(n = 1, mean = muIR1 + wIRt_1 + (thetaIR %*% wIRtm1_1), 
    ...
    pred_IR_subj2[i,,t] <- rmvnorm(n = 1, mean = muIR2 + wIRt_2 + (thetaIR %*% wIRtm1_2), 
                                   sigma = as.matrix(forceSymmetric(SigmaIR)))
    ...
    pred_IR_subj3[i,,t] <- rmvnorm(n = 1, mean = muIR3 + wIRt_3 + (thetaIR %*% wIRtm1_3), 
                                   sigma = as.matrix(forceSymmetric(SigmaIR)))
    ...
    pred_IR_subj4[i,,t] <- rmvnorm(n = 1, mean = muIR4 + wIRt_4 + (thetaIR %*% wIRtm1_4), 
                                   sigma = as.matrix(forceSymmetric(SigmaIR)))
  }
}
...
saveRDS(pred_P_subj1, paste0("pred_P_subj1.rds")) 
saveRDS(pred_P_subj2, paste0("pred_P_subj2.rds")) 
saveRDS(pred_P_subj3, paste0("pred_P_subj3.rds")) 
saveRDS(pred_P_subj4, paste0("pred_P_subj4.rds")) 

saveRDS(pred_A_subj1, paste0("pred_A_subj1.rds")) 
saveRDS(pred_A_subj2, paste0("pred_A_subj2.rds")) 
saveRDS(pred_A_subj3, paste0("pred_A_subj3.rds")) 
saveRDS(pred_A_subj4, paste0("pred_A_subj4.rds")) 

saveRDS(pred_D_subj1, paste0("pred_D_subj1.rds")) 
saveRDS(pred_D_subj2, paste0("pred_D_subj2.rds")) 
saveRDS(pred_D_subj3, paste0("pred_D_subj3.rds")) 
saveRDS(pred_D_subj4, paste0("pred_D_subj4.rds")) 

saveRDS(pred_IR_subj1, paste0("pred_IR_subj1.rds")) 
saveRDS(pred_IR_subj2, paste0("pred_IR_subj2.rds")) 
saveRDS(pred_IR_subj3, paste0("pred_IR_subj3.rds")) 
saveRDS(pred_IR_subj4, paste0("pred_IR_subj4.rds")) 
...
pred.plot <- function(subj = 1, 
  ...
  Pestimate <- apply(pathogen,2,function(x)median(x))
  Aestimate <- apply(antibodies,2,function(x)median(x))
  IRestimate <- matrix(NA,6,pred_time)
  for(i in 1:6){
    IRestimate[i,] <- apply(IRresp[,i,],2,function(x)median(x))
  }
  ...
  # Plot: Pathogen Load
  pdf(file=paste0("Posterior Predictive Sampling - Plots/P_Subject",subj,".pdf"))
  par(mfrow=c(1,1))
  ...
  polygon(c(2:pred_time,rev(2:pred_time)),c(lowerp95[2:pred_time],rev(upperp95[2:pred_time])),
          ...
          border = NA)
  lines(2:pred_time, lowerp95[2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, upperp95[2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, trP[subj,2:pred_time]*generalmaxP, col = "black",lwd = 3, lty=1, type="b")
  lines(2:pred_time, Pestimate[2:pred_time],col = "red",lwd = 3)
  grid(NULL, NULL, col = "white")
  legend("topleft", legend=c("Predicted", "Observed", "95% CI Bounds"), 
         lty = c(1,2,2), lwd = c(3,3,3), col = c("red", "black", "royalblue3"),
         inset = 0.02, cex = 0.85, seg.len=3.5, pch = c(NA,19,NA), bg='white')
  dev.off() 
  
  # Plot: Antibody Levels
  pdf(file=paste0("Posterior Predictive Sampling - Plots/A_Subject",subj,".pdf"))
  par(mfrow=c(1,1))
  ...
  polygon(c(2:pred_time,rev(2:pred_time)),c(lowera95[2:pred_time],rev(uppera95[2:pred_time])),
          col=adjustcolor("blue", alpha.f = 0.25),
          border = NA)
  lines(2:pred_time, lowera95[2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, uppera95[2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, trA[subj,2:pred_time]*generalmaxA, col = "forestgreen",lwd=3, lty=1, type="b")
  lines(2:pred_time, Aestimate[2:pred_time], col = "black", lwd = 3)
  legend("topleft", legend=c("Predicted", "Observed","95% CI Bounds"), 
         lty = c(1,2,2), lwd = c(3,3,3), col = c("black", "forestgreen","royalblue3"),
         inset = 0.02, cex = 0.85, seg.len=3.5, pch = c(NA,19,NA), bg='white')
  dev.off() 
  ...
  for(i in 3:pred_time){
    ...
    df[,i] <- c(tb[1],tb[2],tb[3],1-sum(tb[1:3]))
  }
  
  pdf(file=paste0("Posterior Predictive Sampling - Plots/D_Subject",subj,".pdf"))
  par(mfrow=c(1,1))
  ...
  rect(par("usr")[1], par("usr")[3],
       ...
       col = "gray92") 
  grid(NULL, NULL, col = "white")
  lines(2:pred_time, df[1,2:pred_time], col = "darkorange1", lwd = 3, lty = 1)
  lines(2:pred_time, df[2,2:pred_time], col = "darkorange1", lwd = 3, lty = 2)
  lines(2:pred_time, df[3,2:pred_time], col = "darkorange2", lwd = 3, lty = 3)
  lines(2:pred_time, df[4,2:pred_time], col = "darkorange3", lwd = 3, lty = 4)
  legend("topleft", legend=c("D=1","D=2","D=3","D=4"), 
  ...
  pdf(file=paste0("Posterior Predictive Sampling - Plots/I1_Subject",subj,".pdf"))
  par(mfrow=c(1,1))
  plot(2:pred_time, IRresp[1,1,2:pred_time], 
       xlab = "Time Point",
       ylab = "Prop INF-gamma+ of CD4+",
       main = paste0("Predicted Proportion of INF-gamma+ of CD4+ for Subject ", subj),
       col = "darkgray",
       type = "l",
       xlim = c(2,pred_time),
       ylim = c(0,1),
  ...
  polygon(c(2:pred_time,rev(2:pred_time)),c(lowerIR95[1,2:pred_time],rev(upperIR95[1,2:pred_time])),
          col=adjustcolor("blue", alpha.f = 0.25),
          border = NA)
  lines(2:pred_time, lowerIR95[1,2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, upperIR95[1,2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, trIR[subj,1,2:pred_time], col = "black",lwd = 3, lty=1, type="b")
  lines(2:pred_time, IRestimate[1,2:pred_time], col = "blue",lwd = 3)
  grid(NULL, NULL, col = "white")
  legend("topleft", legend=c("Predicted", "Observed", "95% CI Bounds"), 
         lty = c(1,2,2), lwd = c(3,3,3), col = c("blue", "black", "royalblue3"),
         inset = 0.02, cex = 0.85, seg.len=3.5, pch = c(NA,19,NA), bg='white')
  dev.off() 
  
  # Plot: Proportion of Proliferation of CD4+
  pdf(file=paste0("Posterior Predictive Sampling - Plots/I2_Subject",subj,".pdf"))
  par(mfrow=c(1,1))
  ...
  polygon(c(2:pred_time,rev(2:pred_time)),c(lowerIR95[2,2:pred_time],rev(upperIR95[2,2:pred_time])),
          col=adjustcolor("blue", alpha.f = 0.25),
          border = NA)
  lines(2:pred_time, lowerIR95[2,2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, upperIR95[2,2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, trIR[subj,2,2:pred_time], col = "black",lwd = 3, lty=1, type="b")
  lines(2:pred_time, IRestimate[2,2:pred_time] ,col = "blue",lwd = 3)
  grid(NULL, NULL, col = "white")
  legend("topleft", legend=c("Predicted", "Observed","95% CI Bounds"), 
         lty = c(1,2,2), lwd = c(3,3,3), col = c("blue", "black","royalblue3"),
         inset = 0.02, cex = 0.85, seg.len=3.5, pch = c(NA,19,NA), bg='white')
  ...
  pdf(file=paste0("Posterior Predictive Sampling - Plots/I3_Subject",subj,".pdf"))
  par(mfrow=c(1,1))
  ...
  polygon(c(2:pred_time,rev(2:pred_time)),c(lowerIR95[3,2:pred_time],rev(upperIR95[3,2:pred_time])),
          col=adjustcolor("blue", alpha.f = 0.25),
          border = NA)
  lines(2:pred_time, lowerIR95[3,2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, upperIR95[3,2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, trIR[subj,3,2:pred_time], col = "black",lwd = 3, lty=1, type="b")
  lines(2:pred_time, IRestimate[3,2:pred_time] ,col = "blue",lwd = 3)
  grid(NULL, NULL, col = "white")
  legend("topleft", legend=c("Predicted", "Observed","95% CI Bounds"), 
         lty = c(1,2,2), lwd = c(3,3,3), col = c("blue", "black","royalblue3"),
         inset = 0.02, cex = 0.85, seg.len=3.5, pch = c(NA,19,NA), bg='white')
  ...
  pdf(file=paste0("Posterior Predictive Sampling - Plots/R1_Subject",subj,".pdf"))
  par(mfrow=c(1,1))
  ...
  polygon(c(2:pred_time,rev(2:pred_time)),c(lowerIR95[4,2:pred_time],rev(upperIR95[4,2:pred_time])),
          col=adjustcolor("blue", alpha.f = 0.25),
          border = NA)
  lines(2:pred_time, lowerIR95[4,2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, upperIR95[4,2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, trIR[subj,4,2:pred_time], col = "black",lwd = 3, lty=1, type="b")
  lines(2:pred_time, IRestimate[4,2:pred_time] ,col = "purple",lwd = 3)
  grid(NULL, NULL, col = "white")
  legend("topleft", legend=c("Predicted", "Observed","95% CI Bounds"), 
         lty = c(1,2,2), lwd = c(3,3,3), col = c("purple", "black","royalblue3"),
         inset = 0.02, cex = 0.85, seg.len=3.5, pch = c(NA,19,NA), bg='white')
  dev.off() 
  
  # Plot: Proportion of PD1+ of CD4+
  pdf(file=paste0("Posterior Predictive Sampling - Plots/R2_Subject",subj,".pdf"))
  par(mfrow=c(1,1))
  ...
  polygon(c(2:pred_time,rev(2:pred_time)),c(lowerIR95[5,2:pred_time],rev(upperIR95[5,2:pred_time])),
          col=adjustcolor("blue", alpha.f = 0.25),
          border = NA)
  lines(2:pred_time, lowerIR95[5,2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, upperIR95[5,2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, trIR[subj,5,2:pred_time], col = "black",lwd = 3, lty=1, type="b")
  lines(2:pred_time, IRestimate[5,2:pred_time] ,col = "purple",lwd = 3)
  grid(NULL, NULL, col = "white")
  legend("topleft", legend=c("Predicted","Observed","95% CI Bounds"), 
         lty = c(1,2,2), lwd = c(3,3,3), col = c("purple", "black","royalblue3"),
         inset = 0.02, cex = 0.85, seg.len=3.5, pch = c(NA,19,NA), bg='white')
  ...
  pdf(file=paste0("Posterior Predictive Sampling - Plots/R3_Subject",subj,".pdf"))
  par(mfrow=c(1,1))
  ...
  polygon(c(2:pred_time,rev(2:pred_time)),c(lowerIR95[6,2:pred_time],rev(upperIR95[6,2:pred_time])),
          col=adjustcolor("blue", alpha.f = 0.25),
          border = NA)
  lines(2:pred_time, lowerIR95[6,2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, upperIR95[6,2:pred_time], col = "royalblue3", lwd = 3, lty = 2)
  lines(2:pred_time, trIR[subj,6,2:pred_time], col = "black",lwd = 3, lty=1, type="b")
  lines(2:pred_time, IRestimate[6,2:pred_time] ,col = "purple",lwd = 3)
  grid(NULL, NULL, col = "white")
  legend("topleft", legend=c("Predicted","Observed","95% CI Bounds"), 
         lty = c(1,2,2), lwd = c(3,3,3), col = c("purple", "black","royalblue3"),
         inset = 0.02, cex = 0.85, seg.len=3.5, pch = c(NA,19,NA), bg='white')
