...
clusterSetRNGStream(cl, iseed = 6082022)
...
BayesianModel <- nimbleCode({
  ...
  for(i in 1:ncolsM){
    betaP[i] ~ dnorm(0, sd = sqrt(5))
    betaA[i] ~ dnorm(0, sd = sqrt(5))
    betaD1[i] ~ dnorm(0, sd = 1)
    betaD2[i] ~ dnorm(0, sd = 1)
    betaD3[i] ~ dnorm(0, sd = 1)
    betaI1[i] ~ dnorm(0, sd = sqrt(5))
    betaI2[i] ~ dnorm(0, sd = sqrt(5))
    betaI3[i] ~ dnorm(0, sd = sqrt(5))
    betaR1[i] ~ dnorm(0, sd = sqrt(5))
    betaR2[i] ~ dnorm(0, sd = sqrt(5))
    betaR3[i] ~ dnorm(0, sd = sqrt(5))
  }
  ...
  for(i in 1:N){
    for(t in (first_timepoint[i]+1):last_timepoint[i]){
      ...
      muD1[i,t] <- inprod(M[i,1:ncolsM,t-1],betaD1[1:ncolsM]) + 
        inprod(X[i,1:ncolsX],alphaD[1:ncolsX]) + wD[i,t] + thetaD*wD[i,t-1]
...
parameters <- c("betaP", "alphaP", "sigmaP", "betaA", "alphaA", "sigmaA", 
                "betaD1", "betaD2", "betaD3", "alphaD", "betaI1", "betaI2", 
                "betaI3", "betaR1", "betaR2", "betaR3", "alphaI1", "alphaI2", 
                "alphaI3", "alphaR1", "alphaR2", "alphaR3", "SigmaIR", 
                "P", "D", "A", "IR", "wP", "wA", "wD", "wIR", "thetaP", 
                "thetaA", "thetaD", "thetaIR", "sigma.wP", "sigma.wA", 
                "sigma.wD", "Sigma.wIR")
...
time1 <- Sys.time() 
results_fit <- foreach(x = 1:ncore, .packages = "nimble", .verbose = TRUE) %dopar% {
...
#                   "betaD1", "betaD2", "betaD3", "alphaD", "betaI1", "betaI2", 
#                   "betaI3", "betaR1", "betaR2", "betaR3", "alphaI1", "alphaI2", 
#                   "alphaI3", "alphaR1", "alphaR2", "alphaR3", "SigmaIR", "wP", 
#                   "wA", "wD", "wIR", "thetaP", "thetaA", "thetaD", "thetaIR", 
#                   "sigma.wP", "sigma.wA", "sigma.wD", "Sigma.wIR")
#
### PLOTS ====================
#
## Parameters
...
#                     vec_idx_miss_P,
#                     vec_idx_miss_D[-c(15,33,46,53,54)],
#                     vec_idx_miss_IR),
#          ISB = FALSE,
#          pdf = TRUE,
#          Rhat = TRUE,
